...
export type CalendarSnapshot={tenantId:string;locationId:string|null;view:CalendarView;from:Date;to:Date;rows:CalendarRow[];bookings:CalendarBookingItem[];blocks:CalendarBlockItem[]};
export interface CalendarRepository{snapshot(tenantId:string,view:CalendarView,from:Date,to:Date,locationId?:string):Promise<CalendarSnapshot>;}
...
export class CalendarService{
  ...
  get(tenantId:string,input:{locationId?:string;view?:CalendarView;from:Date|string;to:Date|string}){
    ...
    return this.repo.snapshot(tenantId,view,from,to,input.locationId);
  }
}
