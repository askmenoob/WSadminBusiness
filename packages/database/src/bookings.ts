...
const map=(r:any):Booking=>({id:r.id,tenantId:r.tenant_id,locationId:r.location_id,customerId:r.customer_id,serviceId:r.service_id,staffId:r.staff_id,resourceId:r.resource_id,status:r.status,source:r.source,assignmentMode:r.assignment_mode,optionIds:(r.option_ids??[]).map(String),durationMinutes:r.duration_minutes,basePriceMinor:Number(r.base_price_minor),optionPriceMinor:Number(r.option_price_minor),priceMinor:Number(r.price_minor),currency:r.currency,notes:r.notes,startsAt:r.starts_at,endsAt:r.ends_at,effectiveStartsAt:r.effective_starts_at,effectiveEndsAt:r.effective_ends_at,createdAt:r.created_at,updatedAt:r.updated_at});
...
export function createBookingRepository(pool:Pool):BookingRepository{return{
  async createWithConflictGuard(i:PersistBookingInput){
    ...
    try{
      ...
      for(const optionId of i.optionIds)await c.query('INSERT INTO booking_service_options(tenant_id,booking_id,option_id) VALUES($1,$2,$3)',[i.tenantId,row.id,optionId]);
      await writeAudit(c,{tenantId:i.tenantId,bookingId:row.id,actorUserId:i.actorUserId,eventType:'CREATED',toStatus:row.status,metadata:{startsAt:row.startsAt.toISOString(),staffId:row.staffId,resourceId:row.resourceId,source:row.source,assignmentMode:row.assignmentMode}});
  ...
  async createManyWithConflictGuard(inputs){
    ...
    try{await c.query('BEGIN');
      for(const i of inputs){
        ...
        await writeAudit(c,{tenantId:i.tenantId,bookingId:row.id,actorUserId:i.actorUserId,eventType:'CREATED',toStatus:row.status,metadata:{startsAt:row.startsAt.toISOString(),staffId:row.staffId,resourceId:row.resourceId,source:row.source,assignmentMode:row.assignmentMode,batch:true}});
      }
      await c.query('COMMIT');return rows;
    }catch(e){await c.query('ROLLBACK');throw e;}finally{c.release();}
  },
  async get(t,id){const r=await pool.query('SELECT b.*,ARRAY(SELECT bso.option_id::text FROM booking_service_options bso WHERE bso.tenant_id=b.tenant_id AND bso.booking_id=b.id ORDER BY bso.option_id) AS option_ids FROM bookings b WHERE b.tenant_id=$1 AND b.id=$2',[t,id]);return r.rowCount?map(r.rows[0]):null;},
  async search(t,q){const r=await pool.query(`SELECT b.*,ARRAY(SELECT bso.option_id::text FROM booking_service_options bso WHERE bso.tenant_id=b.tenant_id AND bso.booking_id=b.id ORDER BY bso.option_id) AS option_ids FROM bookings b WHERE b.tenant_id=$1 AND ($2::timestamptz IS NULL OR b.starts_at >= $2) AND ($3::timestamptz IS NULL OR b.starts_at < $3) AND ($4::wsb_booking_status IS NULL OR b.status=$4) AND ($5::wsb_booking_source IS NULL OR b.source=$5) AND ($6::uuid IS NULL OR b.customer_id=$6) AND ($9::uuid IS NULL OR b.location_id=$9) ORDER BY b.starts_at DESC,b.id LIMIT $7 OFFSET $8`,[t,q.from??null,q.to??null,q.status??null,q.source??null,q.customerId??null,q.limit??50,q.offset??0,q.locationId??null]);return r.rows.map(map);},
  async transitionStatus(i:BookingTransitionInput){
    const c=await pool.connect();try{await c.query('BEGIN');const currentQ=await c.query('SELECT b.*,ARRAY(SELECT bso.option_id::text FROM booking_service_options bso WHERE bso.tenant_id=b.tenant_id AND bso.booking_id=b.id ORDER BY bso.option_id) AS option_ids FROM bookings b WHERE b.tenant_id=$1 AND b.id=$2 FOR UPDATE',[i.tenantId,i.bookingId]);if(!currentQ.rowCount)throw new BookingNotFoundError();const current=map(currentQ.rows[0]);if(!i.allowedFrom.includes(current.status))throw new BookingStateError(`cannot transition ${current.status} to ${i.toStatus}`);const updated=await c.query('UPDATE bookings SET status=$3,updated_at=now() WHERE tenant_id=$1 AND id=$2 RETURNING *',[i.tenantId,i.bookingId,i.toStatus]);const row=map(updated.rows[0]);row.optionIds=[...current.optionIds];await writeAudit(c,{tenantId:i.tenantId,bookingId:i.bookingId,actorUserId:i.actorUserId,eventType:`STATUS_${i.toStatus}`,fromStatus:current.status,toStatus:i.toStatus,metadata:{reason:i.reason??null}});await c.query('COMMIT');return row;}catch(e){await c.query('ROLLBACK');throw e;}finally{c.release();}
  },
  async rescheduleWithConflictGuard(i:ReschedulePersistInput){
    const c=await pool.connect();try{await c.query('BEGIN');const currentQ=await c.query('SELECT b.*,ARRAY(SELECT bso.option_id::text FROM booking_service_options bso WHERE bso.tenant_id=b.tenant_id AND bso.booking_id=b.id ORDER BY bso.option_id) AS option_ids FROM bookings b WHERE b.tenant_id=$1 AND b.id=$2 FOR UPDATE',[i.tenantId,i.bookingId]);if(!currentQ.rowCount)throw new BookingNotFoundError();const current=map(currentQ.rows[0]);if(!['PENDING','CONFIRMED'].includes(current.status))throw new BookingStateError(`cannot reschedule ${current.status} booking`);
      ...
      const updated=await c.query(`UPDATE bookings SET location_id=$3,staff_id=$4,resource_id=$5,assignment_mode=$6,starts_at=$7,ends_at=$8,effective_starts_at=$9,effective_ends_at=$10,updated_at=now() WHERE tenant_id=$1 AND id=$2 RETURNING *`,[i.tenantId,i.bookingId,i.locationId,i.staffId,i.resourceId,i.assignmentMode,i.startsAt,i.endsAt,i.effectiveStartsAt,i.effectiveEndsAt]);
      const row=map(updated.rows[0]);row.optionIds=[...current.optionIds];await writeAudit(c,{tenantId:i.tenantId,bookingId:i.bookingId,actorUserId:i.actorUserId,eventType:'RESCHEDULED',fromStatus:current.status,toStatus:current.status,metadata:{from:{startsAt:current.startsAt.toISOString(),staffId:current.staffId,resourceId:current.resourceId},to:{locationId:row.locationId,startsAt:row.startsAt.toISOString(),staffId:row.staffId,resourceId:row.resourceId,assignmentMode:row.assignmentMode}}});await c.query('COMMIT');return row;
    }catch(e){await c.query('ROLLBACK');throw e;}finally{c.release();}
  },
