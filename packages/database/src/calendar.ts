...
export function createCalendarRepository(pool:Pool):CalendarRepository{return{
  async snapshot(tenantId:string,view:CalendarView,from:Date,to:Date,locationId?:string):Promise<CalendarSnapshot>{
    const rowsQuery=view==='staff'
      ?await pool.query(`SELECT id,display_name AS name,booking_capacity AS capacity,photo_url FROM staff_profiles WHERE tenant_id=$1 AND active=true AND ($2::uuid IS NULL OR EXISTS(SELECT 1 FROM staff_locations sl WHERE sl.tenant_id=staff_profiles.tenant_id AND sl.staff_id=staff_profiles.id AND sl.location_id=$2::uuid)) ORDER BY sort_order,display_name`,[tenantId,locationId??null])
      :await pool.query(`SELECT id,name,capacity,type FROM resources WHERE tenant_id=$1 AND active=true AND ($2::uuid IS NULL OR location_id=$2::uuid) ORDER BY sort_order,name`,[tenantId,locationId??null]);
    ...
    if(view==='staff'){
      const blockRows=await pool.query(`SELECT id,staff_id,type,starts_at,ends_at,reason FROM staff_time_blocks WHERE tenant_id=$1 AND starts_at<$3 AND $2<ends_at AND ($4::uuid IS NULL OR EXISTS(SELECT 1 FROM staff_locations sl WHERE sl.tenant_id=staff_time_blocks.tenant_id AND sl.staff_id=staff_time_blocks.staff_id AND sl.location_id=$4::uuid)) ORDER BY starts_at,id`,[tenantId,from,to,locationId??null]);
    ...
    for(const b of controlRows.rows){
      const targets=(b.scope==='TENANT'?rows.map(r=>r.id):[view==='staff'?b.staff_id:b.resource_id].filter(Boolean)).filter((id:any)=>rowIds.has(id));
    ...
    return{tenantId,locationId:locationId??null,view,from,to,rows,bookings,blocks};
  }
};}
