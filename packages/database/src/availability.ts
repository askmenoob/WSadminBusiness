...
export function createAvailabilityRepository(pool:Pool):AvailabilityRepository{return{
  ...
  async getLocation(t,id){const r=await pool.query('SELECT id,timezone,active FROM locations WHERE tenant_id=$1 AND id=$2',[t,id]);return r.rowCount?{id:r.rows[0].id,timezone:r.rows[0].timezone,active:r.rows[0].active}:null;},
  async isServiceAtLocation(t,s,l){const r=await pool.query('SELECT 1 FROM service_locations WHERE tenant_id=$1 AND service_id=$2 AND location_id=$3',[t,s,l]);return Boolean(r.rowCount);},
  ...
  async listEligibleStaff(t,serviceId,staffId,locationId){const r=await pool.query(`SELECT sp.id,sp.active,sp.booking_capacity,sp.sort_order,sp.display_name,sp.photo_url FROM staff_profiles sp JOIN staff_services ss ON ss.tenant_id=sp.tenant_id AND ss.staff_id=sp.id WHERE sp.tenant_id=$1 AND ss.service_id=$2 AND ($3::uuid IS NULL OR sp.id=$3::uuid) AND ($4::uuid IS NULL OR EXISTS(SELECT 1 FROM staff_locations sl WHERE sl.tenant_id=sp.tenant_id AND sl.staff_id=sp.id AND sl.location_id=$4::uuid)) AND sp.active=true ORDER BY sp.sort_order,sp.display_name`,[t,serviceId,staffId??null,locationId??null]);return r.rows.map((x:any):AvailabilityStaff=>({id:x.id,active:x.active,bookingCapacity:x.booking_capacity,sortOrder:x.sort_order,displayName:x.display_name,photoUrl:x.photo_url}));},
  ...
  async listCompatibleResources(t,serviceId,resourceId,requiredType,locationId){const r=await pool.query(`SELECT r.id,r.active,r.capacity,r.type,rs.allocation_priority FROM resources r JOIN resource_services rs ON rs.tenant_id=r.tenant_id AND rs.resource_id=r.id WHERE r.tenant_id=$1 AND rs.service_id=$2 AND ($3::uuid IS NULL OR r.id=$3::uuid) AND ($4::text IS NULL OR r.type=$4::wsb_resource_type) AND ($5::uuid IS NULL OR r.location_id=$5::uuid) AND r.active=true ORDER BY rs.allocation_priority,r.sort_order,r.name`,[t,serviceId,resourceId??null,requiredType??null,locationId??null]);return r.rows.map((x:any):AvailabilityResource=>({id:x.id,active:x.active,capacity:x.capacity,type:x.type,allocationPriority:x.allocation_priority}));},
