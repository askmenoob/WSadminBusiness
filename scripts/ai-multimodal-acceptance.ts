...
async function main(){const pool=createPool(),tenantId=randomUUID();try{
 ...
 const calls:any[]=[];const downstream={async handle(i:any){calls.push(i);return{handled:true,action:'EXECUTE' as const,intent:'BOOK',confidence:.95,reason:'high_confidence_complete',reply:'booking ready',outboundMessageId:'out1',bookingId:'book1'};}};
