  ...
  return <>
    ...
    <div className="app-shell">
      ...
      <main className="app-main" id="main-content" tabIndex={-1}>
        ...
        <div className="topbar"><div className="breadcrumb"><span>WSadmin Business</span><b>/</b><strong>{active}</strong></div><div className="top-actions"><button className="search-button">Search <kbd>⌘K</kbd></button><button className="secondary-button">Help</button>{!systemOwner?<button className="primary-button" onClick={()=>open('Bookings')}>+ New booking</button>:null}{auth.session?<button className="avatar-button" aria-label="Sign out" title={`Sign out ${identity?.email}`} onClick={()=>void logout()}>{userInitials(identity?.displayName,identity?.email)}</button>:<button className="avatar-button" aria-label="UAT account">KN</button>}</div></div>
