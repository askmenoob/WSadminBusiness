...
import { systemGet, uatRole, type BillingSummary, type SystemDashboard } from './api';
...
const myr = (minor: number) => new Intl.NumberFormat('en-MY',{style:'currency',currency:'MYR'}).format(minor/100);
...
export function SystemOwnerWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    try { const[nextDashboard,nextBilling]=await Promise.all([systemGet<SystemDashboard>('/dashboard'),systemGet<BillingSummary>('/billing')]);setDashboard(nextDashboard);setBilling(nextBilling); }
  ...
  if (uatRole !== 'SYSTEM_OWNER') return <section className="workspace-card role-blocked"><span>SO</span><h1>System Owner access required</h1><p>This dashboard is restricted to a verified System Owner session.</p></section>;
  ...
  return <>
    ...
    <div className="metric-grid owner-billing-metrics"><article className="metric-card"><div><small>Monthly recurring revenue</small><strong>{myr(billing?.monthlyRecurringRevenueMinor??0)}</strong></div><span className="metric-note positive">Verified subscriptions</span></article><article className="metric-card"><div><small>Collected subscription revenue</small><strong>{myr(billing?.paidRevenueMinor??0)}</strong></div><span className="metric-note">HitPay charges</span></article><article className="metric-card"><div><small>Past due</small><strong>{number(billing?.pastDueSubscriptions??0)}</strong></div><span className={(billing?.pastDueSubscriptions??0)>0?'metric-note warning':'metric-note'}>Needs attention</span></article><article className="metric-card"><div><small>Pending checkouts</small><strong>{number(billing?.pendingCheckouts??0)}</strong></div><span className="metric-note">Awaiting webhook</span></article></div>
